      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        <Reveal>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <a href="/projects" className="text-sm font-semibold text-[var(--z-navy)] transition hover:opacity-70">
              View all projects →
            </a>
          </div>
        </Reveal>
          ...
          {projects.map((project, index) => (
            <Reveal key={project.title} delay={index * 0.08}>
              <div className="hover-lift rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-gray-200">
                <h3 className="display-font text-3xl font-semibold text-[var(--z-navy)]">
                  {project.title}
                </h3>
